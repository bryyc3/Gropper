...
class SocketManagerService {
    ...
    private init() {
        ...
        manager = SocketManager(socketURL: URL(string: "https://gropper-api-production.up.railway.app/")!, config: [.log(true), .compress, .extraHeaders(["Authorization": "Bearer \(token)"])])
